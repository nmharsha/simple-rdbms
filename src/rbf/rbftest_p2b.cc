...
int RBFTest_private_2b(RecordBasedFileManager *rbfm) {
    ...
    if (headerPageExists) {
        if (readPageCountDiff < 1 || appendPageCountDiff < 1 || writePageCount < 1) {
            cout << "The implementation regarding insertRecord() is not correct1." << endl;
        ...
        if (readPageCountDiff < numRecords) {
        	cout << readPageCount1 << ", " << readPageCount << endl;
        	cout << numRecords << endl;
            cout << "The implementation regarding insertRecord() is not correct2." << endl;
            cout << "***** [FAIL] Test Case Private 2b Failed! *****" << endl;
            rc = rbfm->closeFile(fileHandle);
            rc = rbfm->destroyFile(fileName);
            free(record);
